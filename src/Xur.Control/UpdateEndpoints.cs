...
public static class UpdateEndpoints
{
    public static void MapUpdates(this WebApplication app,Appliance device)
    {
        ...
        async Task<IResult> ApplicationAct(ApplicationUpdateRequest request,bool browser=false)
        {
            ...
            return browser && r.IsSuccessStatusCode ? Results.Redirect(request.Action is "configure" or "development"?"/settings#development-updates":"/updates") : Results.Content(await r.Content.ReadAsStringAsync(),"application/json",statusCode:(int)r.StatusCode);
        }
        ...
        app.MapPost("/application-updates/action",(Func<HttpContext,Task<IResult>>)(async c=> {
            var f=await c.Request.ReadFormAsync();return await ApplicationAct(new(f["action"].ToString(),f["server"].ToString(),f["development"]=="true"),true);
        }));
