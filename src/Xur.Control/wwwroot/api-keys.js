(()=>{
 ...
 async function refresh(){const keys=await api('/api/api-keys');const list=$('api-key-list');list.replaceChildren();if(!keys.length){list.textContent='No API keys yet.';return;}
  for(const k of keys){const row=document.createElement('article');row.className='panel';const title=document.createElement('h3');title.textContent=k.name;const detail=document.createElement('p');detail.textContent=`${k.scope} · ${k.revokedAt?'Revoked':new Date(k.expiresAt)<=new Date()?'Expired':'Active'} · Expires ${date(k.expiresAt)}`;const usage=document.createElement('p');usage.className='secondary-text';usage.textContent=`Created ${date(k.createdAt)} · Last used ${date(k.lastUsedAt)} · ${k.requests} authorized requests${k.lastPath?' · '+k.lastMethod+' '+k.lastPath:''}`;row.append(title,detail,usage);
