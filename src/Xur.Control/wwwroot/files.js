(() => {
 ...
 const link=(text,click)=>{const a=document.createElement('a');a.href='#';a.textContent=text;a.onclick=e=>{e.preventDefault();click();};return a;};
 ...
 async function load(path='',push=true){
  ...
  crumbs.append(link(account?.name||'Home',()=>{filter.value='';load('');}));
  let parent='';for(const segment of path.split('/').filter(Boolean)){parent+=(parent?'/':'')+segment;const target=parent;crumbs.append(document.createTextNode(' / '),link(segment,()=>{filter.value='';load(target);}));}
  try{
