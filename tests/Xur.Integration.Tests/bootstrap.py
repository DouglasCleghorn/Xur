...
with tempfile.TemporaryDirectory(prefix='xur-test-') as temp:
    ...
    client.chmod(0o700)
    env=dict(os.environ,XUR_RUN=str(root/'run'),XUR_PORT='18070',XUR_MODE='Installer',XUR_CONSOLE='stdio',PATH=str(root/'bin')+':'+os.environ['PATH'])
    ...
    try:
        ...
        check(request('/api/install/plan',{})[0]==401,'API disk plan requires authentication')
        ...
        check(request('/install/approve',{})[0]==401,'Anonymous install mutation denied')
        ...
        # Hold discovery in Starting to reproduce the stalled-code report.
        initial=urllib.request.Request('https://127.0.0.1:18433/api/bootstrap',data=json.dumps({'token':code}).encode(),headers={'Content-Type':'application/json'})
        with tls_open(initial,timeout=5) as response: initial_session=json.load(response)['accessToken']
        check(bool(initial_session),'Local code authenticates while storage discovery is still Starting')
        discovery_done=True
        formatted=fixed[:3]+'-'+fixed[3:]
        for _ in range(40):
            if formatted in unix_request('control.sock','/local/login')[1]:break
            time.sleep(.1)
        else:raise AssertionError('Answer token did not replace the initial code')
        code=formatted
        request_with_session=urllib.request.Request('https://127.0.0.1:18433/api/installer',headers={'Authorization':'Bearer '+initial_session})
        ...
        check(status==200 and 'Install Xur' in body,'Browser creates account and receives manager session')
        ...
        try:tls_open(initial,timeout=5);raise AssertionError('Token reused after setup')
        except urllib.error.HTTPError as error:check(error.code==401,'Bootstrap exchange disabled after setup')
        try:tls_open(request_with_session,timeout=5);raise AssertionError('Old setup session reused')
        except urllib.error.HTTPError as error:check(error.code==401,'Old bootstrap sessions invalidated')
        check("Install Xur" in request('/')[1], 'Installation dashboard is available after authentication')
        check(request('/api/profiles')[0]==409,'Profile APIs are disabled in installer mode')
        profile_csrf=re.search(r'name="__RequestVerificationToken" value="([^"]+)"',request('/tailscale')[1]).group(1)
        check(request('/profiles/resume',{'__RequestVerificationToken':profile_csrf})[0]==409,'Installer mode cannot resume workload changes')
        home=request('/')[1]
        check('type="radio"' in home and 'Hidden disk' not in home and 'Answer-file discovery' not in home,'Dashboard offers only available disks without discovery details')
        check(not any(text.lower() in home.lower() for text in ['prototype','qualification','Your disks stay protected','YOUR HARDWARE','signed in','CONTROL CENTER']),'Installer dashboard contains no slogans or redundant session status')
        operation={'id':'test-operation','stage':'Installing','message':'Installing selected disk','updated':'2026-09-12T00:00:00Z'}
        check(request('/')[0]==200 and 'View progress' in request('/')[1],'Dashboard stays available after an operation starts')
        progress=request('/install/progress')[1]
        check('test-operation' in progress and 'Installing selected disk' in progress and 'operation-stage' in progress,'Operation fields render in a structured box')
        operation=None
        ...
        check(request('/install/approve',{})[0]==400,'Authenticated mutation still requires antiforgery token')
        ...
        persisted=urllib.request.Request('https://127.0.0.1:18433/api/installer',headers={'Authorization':'Bearer '+manager})
        ...
        check('Install Xur' in request('/')[1],'Browser manager cookie survives restart')
