...
(async()=>{
 ...
 try{
  ...
  assert.equal((await request.get(tls+'/setup-account/logs',{maxRedirects:0})).status(),302);
  ...
  for(const width of [1440,390,320]) {
   await signup.setViewportSize({width,height:900});
   const eye=signup.getByRole('button',{name:'Show password',exact:true});
   const bounds=await eye.boundingBox();assert(bounds.width>=44&&bounds.height>=44);
   assert(await signup.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
   await eye.click();assert.equal(await signup.locator('[name=password]').getAttribute('type'),'text');
   await signup.getByRole('button',{name:'Hide password',exact:true}).press('Space');
   assert.equal(await signup.locator('[name=password]').getAttribute('type'),'password');
   assert.equal(await signup.locator('[name=password]').inputValue(),password);
  }
  const setupCookie=(await context.cookies()).find(c=>c.name==='xur.session').value;
  // A failed write must leave a usable page and protected diagnostics, then allow retry.
  fs.mkdirSync(path.join(root,'manager-account.json'));
  const failedPost=signup.waitForResponse(r=>r.url()===tls+'/auth/setup');
  await signup.getByRole('button',{name:'Create account',exact:true}).click();assert.equal((await failedPost).status(),500);
  await signup.getByRole('link',{name:'Download setup logs'}).waitFor();
  const setupLogs=await request.get(tls+'/setup-account/logs');assert.equal(setupLogs.status(),200);
  assert(setupLogs.headers()['content-disposition'].includes('xur-setup-logs.txt'));
  const failure=await setupLogs.text();assert(failure.includes('Browser request failed: /auth/setup'));assert(failure.includes('manager-account.json'));assert(!failure.includes(password));assert(!failure.includes(code));assert(!failure.includes(setupCookie));
  fs.rmdirSync(path.join(root,'manager-account.json'));
  await signup.getByRole('link',{name:'Return to account setup'}).click();
  await signup.locator('[name=username]').fill('owner@example.test');await signup.locator('[name=password]').fill(password);
  await signup.getByRole('button',{name:'Create account',exact:true}).click();await signup.waitForURL(tls+'/');
  const staleSetup=await browser.newContext({ignoreHTTPSErrors:true});
  await staleSetup.addCookies([{name:'xur.session',value:setupCookie,url:tls,secure:true}]);
  assert.equal((await staleSetup.request.get(tls+'/setup-account/logs',{maxRedirects:0})).status(),302);await staleSetup.close();
  ...
  const logs=await request.get(tls+'/api/logs',{headers});assert.equal(logs.headers()['content-encoding'],'gzip');assert((await logs.text()).includes(logBody));assert((await logs.text()).includes('Web manager (current process)'));
  ...
  const diagnostic=await request.get(tls+'/api/diagnostics/display');assert.equal(diagnostic.status(),200);const report=await diagnostic.json();assert(report.applicationLogs.includes(logBody));assert(report.displayError);assert.deepEqual(report.inventory,[]);
