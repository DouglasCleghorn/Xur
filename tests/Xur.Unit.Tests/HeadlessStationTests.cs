...
static class HeadlessStationTests
{
    public static async Task Run(Action<bool,string> check)
    {
        ...
        try
        {
            var result=await Processes.Run("bash",["-c",script,"xur",home,"/dev/dri/card7","headless","VK_LOADER_DRIVERS_SELECT=*nvidia*"],10);
            ...
            result=await Processes.Run("bash",["-c",script,"xur",home,"/dev/dri/card7","local",""],10);
